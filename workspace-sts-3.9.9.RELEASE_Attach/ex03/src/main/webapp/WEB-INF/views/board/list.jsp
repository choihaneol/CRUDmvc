            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">  <!-- 패널은 콘텐츠가 있는 박스 형태의 구성요소  -->
                        ...
                        <!-- 등록버튼  -->
                        <button id='regBtn' type="button" class="btn btn-xs pull-right">Register New Board</button> <!-- 부트스트랩 버튼  -->
                        </div>
                        <!-- /.panel-heading -->
                       ...
                       <tr>
							...
							<!-- <td><a href='/board/get?bno=<c:out value="${board.bno}" />'>  --> <!-- 목록페이지에서 각 제목에 링크 설정-->
							<td><a class='move' href='<c:out value="${board.bno}"/>'>      <!--  <a> 태그에 이동하려는 게시물의번호만을 가지게 수정(이벤트처리를 수월하게 하기위해서 <a>태그에 class속성 부여 -->
                ...
                <!-- 페이지번호를 클릭해서 이동할때도 검색조건과 키워드는 같이 전달 -->
...
<script type="text/javascript">
	$(document).ready(function() {   //모든 html 페이지가 화면에 뿌려지고 나서 ready안에 서술된 이벤트들이 동작준비
		...
		//			if (result === ''|| history.state) { //뒤로가기방지 
		//				return;
		//			}
...
//javascript에 페이지번호를 클릭하면 처리하는 부분 추가 
var result = '<c:out value="${result}"/>';
...
$(".move").on("click", function(e) { //게시물을 클릭했을때 이동하도록 하는 이벤트 처리 
